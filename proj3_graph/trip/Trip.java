...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        int currentEdge = segment.get(0);
        int nextEdge = segment.get(1);
        String roadName = "";
        String lastRoad = "";
        String secDirection = "";

        String direction =
                _map.getLabel(currentEdge, nextEdge).direction().fullName();
        String street = _map.getLabel(currentEdge, nextEdge).toString();
        double distance = _map.getLabel(currentEdge, nextEdge).length();

        for (int i = 1; i < segment.size() - 1; i++) {

            Road curRoad = _map.getLabel(segment.get(i), segment.get(i + 1));
            roadName = curRoad.toString();
            secDirection = curRoad.direction().fullName();

            if (roadName.equals(street) && secDirection.equals(direction)) {
                distance += curRoad.length();
            } else {
                System.out.printf("%d. Take %s %s for %.1f miles.%n",
                        seq, street, direction, distance);
                seq += 1;
                direction = secDirection;
                distance = curRoad.length();
                street = roadName;
            }
            Location name = _map.getLabel(segment.get(i + 1));
            lastRoad = name.toString();
        }

        System.out.printf("%d. Take %s %s for %.1f miles to %s.%n",
                seq, roadName, secDirection, distance, lastRoad);
        seq += 1;
        return seq;
    }
