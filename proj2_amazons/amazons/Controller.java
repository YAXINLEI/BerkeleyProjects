...
final class Controller {
    ...
    String readLine() {
        System.out.print("> ");
        System.out.flush();
        return (_input.hasNextLine()) ? _input.nextLine().trim() : null;
    }
    ...
    private Command[] _commands = {
        ...
        new Command("dump$", this::doDump), new Command("new$", this::doNew),
        new Command("auto\\s+(?i)(white|black)", this::setAuto),
        new Command("manual\\s+(?i)(white|black)", this::doManual),
        new Command(String.format("%s-%s\\(%s\\)|%s\\s+%s\\s+%s",
                    SQ, SQ, SQ, SQ, SQ, SQ), this::makeMove),
    };
    ...
    /** Sets to auto mode.
     *  @param mat is used to match user input. */
    private void setAuto(Matcher mat) {
        try {
            if (mat.group(1).toLowerCase().equals("white")) {
                _white = _autoPlayerTemplate.create(WHITE, this);
            } else if (mat.group(1).toLowerCase().equals("black")) {
                _black = _autoPlayerTemplate.create(BLACK, this);
            } else {
                throw error("Wrong setting for auto. Invalid side.");
            }
        } catch (NumberFormatException error) {
            throw error("Wrong setting for auto.");
        }
    }

    /** Sets to player manual mode.
     * @param mat is used to match user input. */
    private void doManual(Matcher mat) {
        try {
            if (mat.group(1).toLowerCase().equals("white")) {
                ...
                throw error("Wrong setting for manual. Invalid side.");
            }
        } catch (NumberFormatException e) {
            throw error("Wrong setting for manual.");
        }
    }

    /** Making moves.
     * @param mat is used to match user input. */
    private void makeMove(Matcher mat) {
        try {
            Move myMove = Move.mv(mat.group());
            if (myMove != null) {
                if (!_board.isLegal(myMove)) {
                    throw error("Illegal move. Caution! ");
                }
                _board.makeMove(myMove);
            }

            if (_board.winner() == BLACK || _board.winner() == WHITE) {
                Piece curWinner = _board.winner();
                if (curWinner == WHITE) {
